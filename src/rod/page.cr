...
module Rod
  ...
  class Page < ::Cdp::Client
    ...
    # Use Page.getResourceTree to list available resources.
    def get_resource(url : String) : Bytes
      frame_id = @frame_id
      raise "Page has no frame ID" unless frame_id

      res = Cdp::Page::GetResourceContent.new(
    ...
    # If object belongs to iframe context, clone page with matching js context.
    def element_from_object(obj : ::Cdp::Runtime::RemoteObject) : Element
      object_id = obj.object_id
      raise "Remote object has no object ID" unless object_id

      id = js_ctx_id_by_object_id(object_id)
      page_ctx_id = get_js_ctx_id
    ...
    # Resolve the JavaScript window context ID for a given remote object ID.
    private def js_ctx_id_by_object_id(object_id : String) : String
      req = ::Cdp::Runtime::CallFunctionOn.new(
      ...
      res = req.call(self)
      if ex = res.exception_details
        raise "Failed to resolve js context ID: #{ex}"
      end

      id = res.result.object_id
      raise "No object ID returned for js context" unless id
      id
    end
