  ...
  describe ".shell" do
    {% if flag?(:linux) || flag?(:openbsd) || flag?(:freebsd) %}
      it "matches getent passwd shell entry" do
        ...
        Rod::Util::Utils.shell.should eq(entry[6])
      end
    {% elsif flag?(:android) %}
      it "uses SHELL environment variable" do
        ...
        Rod::Util::Utils.shell.should eq(shell)
      end
    {% elsif flag?(:darwin) %}
      it "matches dscl UserShell output" do
        user = ENV["USER"]?
        if user.nil? || user.empty?
          expect_raises(Exception) { Rod::Util::Utils.shell }
          next
        end
        ...
        Rod::Util::Utils.shell.should eq(matched.not_nil![1])
      end
    {% elsif flag?(:win32) %}
      it "uses COMSPEC with cmd.exe fallback" do
        expected = ENV["COMSPEC"]? || "cmd.exe"
        Rod::Util::Utils.shell.should eq(expected)
      end
    {% end %}
  end
end
