...
describe "cdp client format parity" do
  it "formats request/response/event strings like Go cdp TestFormat" do
    req = Rod::Util::Cdp::Request.new(123, "test", JSON.parse("1"), "000000001234")
    req.to_s.should eq("=> #123 @00000000 test 1")

    res = Rod::Util::Cdp::Response.new(0, JSON.parse("11"), nil)
    res.to_s.should eq("<= #0 11")

    err_res = Rod::Util::Cdp::Response.new(0, nil, Rod::Util::Cdp::Error.new(0, ""))
    err_res.to_s.should eq("<= #0 error: {\"code\":0,\"message\":\"\",\"data\":\"\"}")

    evt = Rod::Util::Cdp::Event.new("event", JSON.parse("11"), nil)
