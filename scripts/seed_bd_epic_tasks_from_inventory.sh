...
# Speed optimization: run one sync at end. Some bd versions don't support
# --no-auto-flush, so detect support at runtime.
BD_FLAGS=(--silent)
if bd create --help 2>&1 | rg -q -- "--no-auto-flush"; then
  BD_FLAGS+=(--no-auto-flush)
fi
...
# One final export/sync after bulk creation when supported.
if bd --help 2>&1 | rg -q '^\s+sync\s'; then
  bd sync
  echo "Completed: epic + ${total} child tasks created and synced."
else
  echo "Completed: epic + ${total} child tasks created."
fi
